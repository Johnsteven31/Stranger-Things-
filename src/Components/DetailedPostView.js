...
const DetailedPostView = () => {
    ...
    const[detailedSpecifcPost, setDetailedSpecficPost] = useState({})
    ...
    useEffect(()=>{
        async function findSpecificPost () {
            try {
                ...
                setDetailedSpecficPost (specificPost);
            } catch (error) {
                console.log (error);
            }
        }
    ...
    return (
        <div>
            ...
            {
                toggleEditForm ? <EditPost indivPost={detailedSpecifcPost} setProfileData = {setProfileData} handleToggleEditForm=
            ...
            <div>
                <h3>Post Info:</h3>
                {
                    detailedSpecifcPost.title ? <p>{detailedSpecifcPost.title}</p> : <p>Untitled Post</p>
                }
            </div>
        </div>
    )
